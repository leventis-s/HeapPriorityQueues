/* Defines functions of the HeapPQueue type
 */
...
void HeapPQueue::enqueue(const DataPoint& data) {
    int n = logicalSize + 1; //leave one space open at the beginning
    if (n == allocatedSize) {
        growArray();
    }
    elems[n] = data;
    logicalSize++;
    /* Bubble up if element has lower weight than its parent*/
    while (n > 1 && elems[n/2].weight > data.weight) {
        swap(elems[n/2], elems[n]);
        n = n/2; //update new location
    }
}
...
/* Find the child of lower weight */
int HeapPQueue::findLowest(int n) {
    int lowest = n;
    if (elems[n * 2].weight < elems[lowest].weight) {
        lowest = n * 2;
    }
    if ((n * 2) + 1 <= logicalSize && elems[(n * 2) + 1].weight < elems[lowest].weight) {
        lowest = (n * 2) + 1;
    }
    return lowest;
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    logicalSize--;
    while (n * 2 <= logicalSize) {
        int lowest = findLowest(n);
        if (lowest == n) {
            return elems[logicalSize + 1];
        }
        /* Bubble down if element has larger weight than one of its children */
        else {
            swap(elems[n], elems[lowest]);
        }
        n = lowest;
    }
    return elems[logicalSize + 1];
...
STUDENT_TEST("Can work with elements tied in weight and elements with weight 0.") {
    HeapPQueue pq;

    /* Add these items in this order. */
    DataPoint one   = { "First",  0 };
    DataPoint two   = { "Second", 0 };
    DataPoint three = { "Third",  1 };
    DataPoint four = { "Fourth",  1 };
