/* Apportion takes in a map of "states" and their weights.
 * Then it returns a map with the proper allocated amount
 * of seats per state.
 */
...
/* Take in states with weights. Give each state a seat and then
 * properly allocate the rest based on their weight.
 */
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    if (numSeats < populations.size()) {
        error("Fewer seats than states.");
    }
    for (string state: populations) {
        apportion[state] = 1; //each state needs at least one seat
        numSeats--;
        double weight = -(populations[state])/sqrt(2);
        elems.enqueue({state, weight});
    }
    while (numSeats > 0) {
        string nextState = elems.dequeue().name; //get state of highest priority
        apportion[nextState] += 1;
        numSeats--;
        int currentSeats = apportion[nextState];
        elems.enqueue({nextState, -(populations[nextState])/sqrt(currentSeats * (currentSeats + 1))});
    }
    return apportion;
}
...
STUDENT_TEST("Works on empty set.") {
    /* Fall 2020 */
    ...
    EXPECT_EQUAL(apportion(populations, 0), seats);
}
